...
function mediafireDownloader(url) {
    return new Promise(async (resolve, reject) => {
        axios.get(url).then(({ data }) => {
            ...
            filesize = $('a#downloadButton').text().replace('Download', '').replace('(', '').replace(')', '').eplace('\n', '').replace('\n', '').replace('                         ', '')
            seplit = link.split('/')
            title = seplit[5]
            mime = title.split('.')[1]
...
function youtube(link, kualitas){
	return new Promise((resolve, reject) => {
...
module.exports = { tiktokDownloader, instaDownloader, facebookDownloader, zippyDownloader, youtube }
